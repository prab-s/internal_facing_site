...
function buildFullChartTooltipFormatter() {
  return (params) => {
    ...
    const lines = [`Airflow: ${formatNumericValue(flowValue)}`];
...
function buildEfficiencyAndPermissibleSeries(points, chartTheme, includeDragHandles, { permissibleLabelColor = null } = {}) {
  const series = [];
